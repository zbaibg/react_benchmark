...
# 3. Submit QM Workers (max 40)
if [ "$QM_COUNT" -gt 0 ]; then
    LIMIT=$((QM_COUNT - 1))
    echo "Submitting $QM_COUNT workers for QM methods (16 cores, array 0-$LIMIT)..."
    sbatch --array=0-$LIMIT worker_qm.slurm
else
    echo "No tasks for QM methods."
fi

# 4. Submit Other Workers (max 300)
if [ "$OTHER_COUNT" -gt 0 ]; then
    LIMIT=$((OTHER_COUNT - 1))
    echo "Submitting $OTHER_COUNT workers for other runs (1 cores, array 0-$LIMIT)..."
