...
# Accept both bare names ("MIm", ...) and the "_monomer" suffix variant written by
# parse_min_last_convergence.py (e.g. "MIm_monomer", "MeOH_monomer").
_MONOMER_REF_KEYS = ("MIm", "MImH", "MImH2", "MeOH", "MeOH2", "H")
_MONOMER_REF_SYSTEM_NAME_TO_KEY: Dict[str, str] = {}
_MONOMER_REF_DISPLAY_NAME: Dict[str, str] = {
    "MIm": "MIm_monomer",
    "MImH": "MImH_monomer",
    "MImH2": "MImH2_monomer",
    "MeOH": "MeOH_monomer",
...
for _k in _MONOMER_REF_KEYS:
    _MONOMER_REF_SYSTEM_NAME_TO_KEY[_k] = _k
    _MONOMER_REF_SYSTEM_NAME_TO_KEY[f"{_k}_monomer"] = _k
_MONOMER_REF_SYSTEM_NAME_TO_KEY["H_monomer"] = "H"

_MONOMER_H_REACTION_TYPE = "monomer Deprotonation H"
_MONOMER_H_REACTIONS: Tuple[Tuple[str, str], ...] = (
    ("MImH", "MImH2"),
    ("MIm", "MImH"),
    ("MeOH", "MeOH2"),
)

# When the "H" monomer row is missing from the TSV, use 0 kcal/mol as the proton
# reference energy (matches the convention in parse_min_last_convergence.py, where
# REFERENCE_H_SYSTEM_NAME="H" is written with Etot=0). Without this, every
# Deprotonation (-H+) edge would have dE_rxn = N/A.
_DEFAULT_H_REF_KCAL = 0.0
...
def load_monomer_refs_kcal_from_tsv(path: Path) -> Dict[str, float]:
    ...
    Accepts both bare names and the ``*_monomer`` suffix variant (as written by
    ``parse_min_last_convergence.py``). Output keys are always the bare names
    (``MIm``, ``MImH``, ``MeOH``, ``H``). If no ``H`` row is present, falls back
    to ``_DEFAULT_H_REF_KCAL`` (0 kcal/mol, proton-reference convention).
    """
    df = pd.read_csv(path, sep="\t", dtype=str, keep_default_na=False)
    df.columns = [str(c).strip() for c in df.columns]
    if "system" not in df.columns or "Etot_last_cycle_kcal_mol" not in df.columns:
        return {"H": _DEFAULT_H_REF_KCAL}
    out: Dict[str, float] = {}
    for _, r in df.iterrows():
        name = str(r["system"]).strip()
        key = _MONOMER_REF_SYSTEM_NAME_TO_KEY.get(name)
        if key is None:
            continue
...
def _monomer_species_to_latex_math(name: str) -> str:
    mapping = {
        "MIm_monomer": r"\mathrm{MIm}",
        "MImH_monomer": r"\mathrm{MImH}",
        "MImH2_monomer": r"\mathrm{MImH_2}",
        "MeOH_monomer": r"\mathrm{MeOH}",
        "MeOH2_monomer": r"\mathrm{MeOH_2}",
        "H": r"\mathrm{H}",
        "H_monomer": r"\mathrm{H}",
    }
    if name in mapping:
        return mapping[name]
    return rf"\text{{{_latex_escape(name)}}}"
...
def balanced_reaction_latex(edge_type: str, fu: str, fv: str) -> str:
    ...
    if edge_type == _MONOMER_H_REACTION_TYPE:
        return (
            rf"{_monomer_species_to_latex_math(fu)} + \mathrm{{H}}"
            rf" \rightarrow {_monomer_species_to_latex_math(fv)}"
        )
...
def collect_monomer_h_reaction_rows(ref_kcal: Dict[str, float]) -> List[ReactionDERow]:
    """Synthetic monomer H-addition reactions, exported as a separate ΔE table."""
    if not ref_kcal:
        return []
    e_h = ref_kcal.get("H")
    if e_h is None:
    ...
    out: List[ReactionDERow] = []
    next_id = -1
    for reactant_key, product_key in _MONOMER_H_REACTIONS:
        eu = ref_kcal.get(reactant_key)
        ev = ref_kcal.get(product_key)
        if eu is None or ev is None:
            continue
        fu = _MONOMER_REF_DISPLAY_NAME[reactant_key]
        fv = _MONOMER_REF_DISPLAY_NAME[product_key]
        de_cluster = ev - eu
        de_rxn = ev - eu - e_h
        out.append(
            (
                _MONOMER_H_REACTION_TYPE,
                next_id,
                next_id - 1,
                fu,
                fv,
...
def print_reaction_dE_table(
    ...
    *,
    ...
    include_monomer_h_notes: bool = False,
) -> None:
    ...
    if from_convergence_tsv:
        ...
        print(
            ...
            "(rows system = MIm, MImH, MImH2, MeOH, MeOH2, H); N/A if a needed monomer is missing.",
            file=out_stream,
        )
        print(
        ...
        if include_monomer_h_notes:
            print(
                "#   Monomer +H:      E(product) - E(reactant) - E(H); rows are appended below the network reactions.",
                file=out_stream,
            )
            print(
                "#   Candidate monomer +H reactions: MImH_monomer + H -> MImH2_monomer; "
                "MIm_monomer + H -> MImH_monomer; MeOH_monomer + H -> MeOH2_monomer.",
                file=out_stream,
            )
        if ref_monomer_kcal:
            print("# Monomer E_tot reference (kcal/mol) from TSV:", file=out_stream)
            for k in ("MImH2", "MImH", "MIm", "MeOH2", "MeOH", "H"):
...
def _latex_document_reaction_dE_for_type(
    ...
    *,
    ...
    lines: List[str] = []
    monomer_h_table = reaction_type == _MONOMER_H_REACTION_TYPE
    ...
    if from_convergence_tsv:
        ...
        if monomer_h_table:
            cap = (
                r"\captionof{table}{"
                + dlabel
                + r" for monomer H-addition reactions from \texttt{min\_last\_convergence.tsv}."
                + r"}"
            )
        else:
            cap = (
    ...
    if monomer_h_table:
        lines.append(r"\begin{longtable}{r p{0.74\textwidth} r}")
    else:
        lines.append(
    ...
    if monomer_h_table:
        lines.append(r"\# & balanced reaction & " + colhdr)
    else:
        lines.append(
    ...
    for i, (_et, iu, iv, fu, fv, eu, ev, de, de_rxn) in enumerate(rows, start=1):
        ...
        if monomer_h_table:
            lines.append(f"{i:d} & {formcol} & {ddr} \\\\")
        else:
            idcol = rf"$\mathrm{{ID}}\,{iu} \rightarrow \mathrm{{ID}}\,{iv}$"
            lines.append(f"{i:d} & {idcol} & {formcol} & {ddr} \\\\")

    lines.append(r"\end{longtable}")
    if from_convergence_tsv and monomer_h_table:
        lines.append(
            r"\textit{Note:} Values use monomer total energies from the same TSV and "
            r"$\Delta E_{\mathrm{rxn}} = E(\mathrm{product}) - E(\mathrm{reactant}) - E(\mathrm{H})$."
        )
    elif from_convergence_tsv:
        lines.append(
...
def main() -> None:
    ...
    col_w = 1.6
    charge_dx = col_w * 0.26

    for _, row in df.iterrows():
        y_pos = row["MeOH"] * 24 + row["Charge"] * 5
        x_pos = row["im_total"] * col_w + row["Charge"] * charge_dx
    ...
    ref_monomer_kcal: Optional[Dict[str, float]] = None
    monomer_h_rows: List[ReactionDERow] = []
    if from_convergence_tsv:
        ...
        monomer_h_rows = collect_monomer_h_reaction_rows(ref_monomer_kcal)
    else:
    ...
    if not args.no_print_reactions:
        stdout_rows = dE_rows
        include_monomer_h_notes = False
        if from_convergence_tsv and monomer_h_rows:
            stdout_rows = dE_rows + monomer_h_rows
            include_monomer_h_notes = True
        print_reaction_dE_table(
            stdout_rows,
            out_stream=sys.stdout,
            from_convergence_tsv=from_convergence_tsv,
            ref_monomer_kcal=ref_monomer_kcal,
            include_monomer_h_notes=include_monomer_h_notes,
        )
    ...
    if not args.no_de_table_png:
        ...
        try:
            ...
            if monomer_h_rows:
                monomer_paths = write_reaction_dE_latex_pngs_by_type(
                    monomer_h_rows,
                    de_out.parent / "react_net_dE.png",
                    dpi=args.latex_dpi,
                    from_convergence_tsv=from_convergence_tsv,
                    ref_monomer_kcal=ref_monomer_kcal,
                )
                for p in monomer_paths:
                    print(f"Wrote LaTeX ΔE table PNG to {p}")
    ...
    for j in range(max_im + 1):
        x_c = j * col_w
        ax.axvspan(
            x_c - col_w / 2, x_c + col_w / 2,
            facecolor=col_colors[j % 2], alpha=0.35, zorder=0,
        )
        ax.text(
            x_c,
            y_bot_band,
    ...
    plt.xticks(
        [i * col_w for i in range(max_im + 1)],
        [str(i) for i in range(max_im + 1)],
        fontsize=11,
        color="#555555",
    )
    plt.yticks([])

    plt.xlim(-3.4, max_im * col_w + 1.0)
