"""Batch path.xyz last-frame Zn–ligand distances (standalone).

The same analysis is integrated into ``qm_minimize/parse_min_last_convergence.py`` TSV columns
``max_Zn-Nmin_MIM(Å)``, ``max_Zn-O_MeOH(Å)``, ``Flag``; prefer that script for run summaries.
"""
...
# Use your fragment identification script
import sys
sys.path.insert(0, str(TOOLS_DIR))
import zif_meoh_assign_name as zma  # xyz_to_mda will assign resname / name
...
ZN_N_SUSPECT = 2.5  # Å, mark if the maximum Zn–Nmin for imidazole is greater than this
ZN_O_SUSPECT = 2.5  # Å, mark if the maximum Zn–O for MeOH is greater than this
...
def main():
    ...
    for base in BASE_DIRS:
        pattern = os.path.join(base, "qm_minimize", "run35", "*", "path.xyz")
