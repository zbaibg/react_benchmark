...
# Try each start dir until software.yaml is found:
#   1) SLURM_SUBMIT_DIR  — sbatch (BASH_SOURCE is often a spool copy)
#   2) dirname(BASH_SOURCE) — bash prepare.sh / bash /path/to/prepare.sh
#   3) PWD               — fallback
_REPO_CANDIDATE=""
_BASH_SRC_DIR=""
if [ -n "${BASH_SOURCE[0]:-}" ]; then
  _BASH_SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" || _BASH_SRC_DIR=""
fi
for _start in \
  "${SLURM_SUBMIT_DIR:-}" \
  "${_BASH_SRC_DIR:-}" \
  "${PWD:-}"; do
  [ -n "$_start" ] && [ -d "$_start" ] || continue
  _cand="$_start"
  while [ "$_cand" != "/" ] && [ ! -f "$_cand/software.yaml" ]; do
    _cand="$(dirname "$_cand")"
  done
  if [ -f "$_cand/software.yaml" ]; then
    _REPO_CANDIDATE="$_cand"
    break
  fi
done
if [ ! -f "${_REPO_CANDIDATE:-}/software.yaml" ]; then
  echo "ERROR: could not locate repo root (software.yaml)" >&2
  echo "  tried from: SLURM_SUBMIT_DIR='${SLURM_SUBMIT_DIR:-}' BASH_SOURCE='${BASH_SOURCE[0]:-}' PWD='${PWD:-}'" >&2
  exit 1
fi
...
node_partition="pre"
core_number_to_apply=%CORE_NUMBER%
mem_per_cpu=6000M

##### Clean and initialization
clean_before_prepare=false
SKIP_PRMTOP_HARMONICRST_NDX_GENERATION=%SKIP_PRMTOP_HARMONICRST_NDX_GENERATION%

##### Settings regarding creating MOL topology
MOL_xyz=%MOL_XYZ%
RST_SOURCE_DIR=%RST_SOURCE_DIR%
PRMTOP_SOURCE_DIR=%PRMTOP_SOURCE_DIR%
charge_MOL=%CHARGE_MOL%
MOL_mask="%MOL_MASK%"
...
nfft=auto
process_ls=("min")
min_maxcyc=%MIN_MAXCYC%
use_dlfind_for_min=%USE_DLFIND_FOR_MIN%
...
expand_nh_oh_bond_radius=%EXPAND_NH_OH_BOND_RADIUS%
delete_wrong_bonds=%DELETE_WRONG_BONDS%

declare -A open_qm
open_qm["min"]=%OPEN_QM_MIN%
...
&dftbplus
  ...
  mixer = \"BROYDEN\" 
  mdftb= %MDFTB%,
  mdftb_scale= %MDFTB_SCALE%,
  skroot= '%SKROOT%',
/
&xtb
  qm_level='%QM_LEVEL%',
  maxiter=250,
  mmhardness=%MM_HARDNESS%,
/
&orc
  use_template=1,
/
...
function create_sbatch() {
  ...
  # If ambertools_sh is set, source it inside the container before sander/cpptraj.
  local amber_setup=""
  if [ -n "${AMBER_SH:-}" ]; then
    amber_setup="source \"$AMBER_SH\" &&"
  fi
...
apptainer exec -e \\
  ...
  "$APPTAINER_SIF" bash -c "\\
    ...
    ulimit -s unlimited &&
    export PATH=\\"\\\$PWD:\\\$PATH:"$ORCA_PATH"/\\" &&
    $amber_setup
    $run_commands
    "
rm -rf "\$TMPDIR"
...
function main() {
    ...
    if [ "${SKIP_PRMTOP_HARMONICRST_NDX_GENERATION}" != true ]; then
        ...
        if [ -d "$RST_SOURCE_DIR" ]; then
            ...
            if [ -f "$RST_SOURCE_DIR/%PRMTOP_SOURCE_FILE%" ]; then
                 cp "$RST_SOURCE_DIR/%PRMTOP_SOURCE_FILE%" ./box_orig.prmtop
            elif [ -f "$PRMTOP_SOURCE_DIR/%PRMTOP_SOURCE_FILE%" ]; then
                 cp "$PRMTOP_SOURCE_DIR/%PRMTOP_SOURCE_FILE%" ./box_orig.prmtop
            else
                 echo "ERROR: %PRMTOP_SOURCE_FILE% not found in $RST_SOURCE_DIR or $PRMTOP_SOURCE_DIR"
                 exit 1
            fi

            if [ -f "$RST_SOURCE_DIR/%RST_SOURCE_FILE%" ]; then
                cp "$RST_SOURCE_DIR/%RST_SOURCE_FILE%" ./min_orig.rst
            else
                echo "Warning: %RST_SOURCE_FILE% not found in $RST_SOURCE_DIR. Trying to find any .rst"
                exit 1
            fi
        else
