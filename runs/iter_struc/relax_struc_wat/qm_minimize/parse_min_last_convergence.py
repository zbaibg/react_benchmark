...
Output columns:
  ...
  without DL-FIND). Folders ``H`` / ``H_monomer`` are forced to 0 (reference proton). N/A if unknown.
  Etot_loose_criteria_cycle — same Etot (kcal/mol) for the first loose-criteria cycle; N/A
  if that cycle is missing or loose criteria never met. For ``H`` / ``H_monomer``, both Etot
...
adapted for ``relax_struc_wat``): ``max_Zn-Nmin_Im(Å)`` and ``max_Zn-O_Wat(Å)`` (Å, three decimals),
plus ``Flag`` (``Im_far`` / ``Wat_far`` if above ``ZN_N_SUSPECT`` / ``ZN_O_SUSPECT``, 2.5 Å by default).
Ligand residues are ``IM-`` / ``IMH`` (imidazole); solvent is ``WAT``. If ``min.out`` contains DL-FIND
abort text (``DL-Find encountered an error``, ``DL-FIND ERROR:``), ``dlfind error`` is appended
to ``Flag`` (comma-separated).
Requires ``tools/zif_meoh_assign_name`` (MDAnalysis).
"""
...
# Simultaneous four-criterion targets (e.g. run41 1Zn_0Im-_0ImH_6Wat).
...
# Geometry screening on path.xyz last frame (Im-/ImH + WAT; see check_qm_minimize.py).
ZN_N_SUSPECT = 2.5  # Å → Flag includes Im_far if max Zn–Nmin(Im-/ImH) exceeds this
ZN_O_SUSPECT = 2.5  # Å → Flag includes Wat_far if max Zn–O(WAT) exceeds this

# Residue names from tools/zif_meoh_assign_name for demethylated (water) complexes.
IM_LIKE_RESNAMES = frozenset({"IM-", "IMH", "MIM", "MIH"})
WAT_RESNAMES = frozenset({"WAT"})

TSV_COL_MAX_ZN_NIM = "max_Zn-Nmin_Im(Å)"
TSV_COL_MAX_ZN_OWAT = "max_Zn-O_Wat(Å)"
TSV_COL_FLAG = "Flag"
...
def _repo_root() -> Path:
    """Walk parents until ``software.yaml`` (repo root); fall back to parents[4]."""
    here = Path(__file__).resolve().parent
    for cand in (here, *here.parents):
        if (cand / "software.yaml").is_file():
            return cand
    return Path(__file__).resolve().parents[4]


def _import_zif_meoh_assign_name():
    """Load zif_meoh_assign_name from repo ``tools`` (one-time)."""
...
def analyze_zn_ligand_last_frame(path_xyz: Path) -> tuple[float | None, float | None, str]:
    """Parse last path.xyz frame via zif_meoh_assign_name: (max Zn–Nmin Im, max Zn–O Wat, error tag).
    ...
    try:
        with os.fdopen(fd, "w") as tf:
            tf.write(frame_txt)
        # Prefer expanded N–H/O–H guessing for IMH; fall back if bond expansion merges fragments.
        try:
            u = zma.xyz_to_mda(tmp, expand_nh_oh_radius=True, delete_wrong_bonds=True)
    ...
    im_residues = [r for r in u.residues if r.resname in IM_LIKE_RESNAMES]
    wat_residues = [r for r in u.residues if r.resname in WAT_RESNAMES]

    im_zn_nmins: list[float] = []
    for res in im_residues:
        ...
        if min_d is not None:
            im_zn_nmins.append(min_d)

    wat_zn_os: list[float] = []
    for res in wat_residues:
        # zif_meoh_assign_name names WAT oxygen ``O`` (MeOH uses ``O1``).
        o = res.atoms.select_atoms("name O or name O1")
        if len(o) == 0:
            o = res.atoms.select_atoms("element O")
        if len(o) == 0:
            continue
        wat_zn_os.append(_dist3(zn_pos, o.positions[0]))

    max_im = max(im_zn_nmins) if im_zn_nmins else None
    max_wat = max(wat_zn_os) if wat_zn_os else None
    return max_im, max_wat, ""


def geometry_tsv_fields(sys_dir: Path, system_rel: str, na: str) -> dict[str, str]:
    """Three TSV columns from path.xyz last frame: max Zn–Nmin(Im), max Zn–O(Wat), Flag."""
    if "_monomer" in system_rel:
        return {
            TSV_COL_MAX_ZN_NIM: na,
            TSV_COL_MAX_ZN_OWAT: na,
    ...
    try:
        max_im, max_wat, err = analyze_zn_ligand_last_frame(path_xyz)
    except Exception as e:  # noqa: BLE001
        return {
            TSV_COL_MAX_ZN_NIM: na,
            TSV_COL_MAX_ZN_OWAT: na,
    ...
    if max_im is None and max_wat is None:
        return {
            TSV_COL_MAX_ZN_NIM: na,
            TSV_COL_MAX_ZN_OWAT: na,
            TSV_COL_FLAG: "no_Im_Wat",
        }

    flags: list[str] = []
    if max_im is not None and max_im > ZN_N_SUSPECT:
        flags.append("Im_far")
    if max_wat is not None and max_wat > ZN_O_SUSPECT:
        flags.append("Wat_far")

    return {
        TSV_COL_MAX_ZN_NIM: na if max_im is None else f"{max_im:.3f}",
        TSV_COL_MAX_ZN_OWAT: na if max_wat is None else f"{max_wat:.3f}",
...
# Reference proton folder names: atomic H is taken as 0 kcal/mol (no reliable parse from failed min.out).
REFERENCE_H_SYSTEM_NAMES = frozenset({"H", "H_monomer"})
...
def apply_reference_h_etot(rec: dict[str, object], system_rel: str) -> None:
    if system_rel not in REFERENCE_H_SYSTEM_NAMES:
        return
