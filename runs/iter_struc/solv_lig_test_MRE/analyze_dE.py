#!/usr/bin/env python3
"""Ligand-exchange energies and solute/solvent MRE decomposition.

dE.csv  : ZnLSolv5 + Solv -> ZnSolv6 + L
MRE.csv : E(complex) = E(solute) + E(solvent) + I(solvent,solute)
"""
...
# Reaction MRE split for ZnLSolv5 + Solv -> ZnSolv6 + L:
#   dE = dE(solute) + dE(solvent) + dE(I)
DE_COMPONENTS = (
    "dE",
    "dE(solute)",
    "dE(solvent)",
...
def _pick_energy(
    e_raw: Optional[float],
    e_bsse: Optional[float],
    has_bsse: bool,
    use_bsse: bool,
) -> Optional[float]:
    if use_bsse:
...
def _region_energy(
    ...
    """Energy of complex / solute / solvent fragment for one complex label."""
    if source_base_dir == "qm_minimize":
        if region != "complex":
            return None
        return get_Etot_amber(os.path.join(run_path, complex_name, "min.out"))

    label = complex_name if region == "complex" else f"{complex_name}_{region}"
    e_raw, e_bsse, has_bsse = get_complex_energies(run_path, label)
    return _pick_energy(e_raw, e_bsse, has_bsse, use_bsse)
...
def _collect_reaction_rows_for_run(
    ...
    """Reaction energies plus MRE solute/solvent/I contributions.

    For ZnLSolv5 + Solv -> ZnSolv6 + L:
      dE         = E(ZnSolv6) + E(L) - E(ZnLSolv5) - E(Solv)
      dE(solute) = E_solute(ZnSolv6) + E(L) - E_solute(ZnLSolv5)
      dE(solvent)= E_solvent(ZnSolv6) - E_solvent(ZnLSolv5) - E(Solv)
      dE(I)      = I(ZnSolv6) - I(ZnLSolv5)
    with dE = dE(solute) + dE(solvent) + dE(I).
    """
    monomer_refs = _collect_monomer_refs(run_path)
    complex_names = _iter_complex_names(run_path, source_base_dir)
    if not complex_names:
        return {}

    complex_infos = {
        formula: info
        for formula in complex_names
    ...
    for formula, info in complex_infos.items():
        ...
        e_from = _region_energy(
            run_path, formula, "complex", use_bsse, source_base_dir
        )
        e_to = _region_energy(
            run_path, zn6, "complex", use_bsse, source_base_dir
        )
        ...
        base_key = (REACTION_TYPE, from_disp, to_disp, balanced)

        # ZnLSolv5 + Solv -> ZnSolv6 + L
        rows[base_key + ("dE",)] = e_to + e_l - e_from - e_solv

        e_from_solute = _region_energy(
            run_path, formula, "solute", use_bsse, source_base_dir
        )
        e_to_solute = _region_energy(
            run_path, zn6, "solute", use_bsse, source_base_dir
        )
        e_from_solvent = _region_energy(
            run_path, formula, "solvent", use_bsse, source_base_dir
        )
        e_to_solvent = _region_energy(
            run_path, zn6, "solvent", use_bsse, source_base_dir
        )

        if e_from_solute is not None and e_to_solute is not None:
            rows[base_key + ("dE(solute)",)] = (
                e_to_solute + e_l - e_from_solute
            )
        if e_from_solvent is not None and e_to_solvent is not None:
            rows[base_key + ("dE(solvent)",)] = (
                e_to_solvent - e_from_solvent - e_solv
            )
        if (
            e_from_solute is not None
            and e_to_solute is not None
            and e_from_solvent is not None
            and e_to_solvent is not None
        ):
            i_from = e_from - e_from_solute - e_from_solvent
            i_to = e_to - e_to_solute - e_to_solvent
            rows[base_key + ("dE(I)",)] = i_to - i_from

    return rows


def _collect_mre_rows_for_run(
    run_path: str,
    source_base_dir: str,
    use_bsse: bool,
) -> Dict[Tuple[str, str], float]:
    """Two-body MRE: E(complex) = E(solute) + E(solvent) + I(solvent,solute)."""
    if source_base_dir == "qm_minimize":
        return {}
    ...
    for complex_name in _iter_complex_names(run_path, source_base_dir):
        e_raw, e_bsse, has_bsse = get_complex_energies(run_path, complex_name)
        e_solute_raw, e_solute_bsse, has_solute_bsse = get_complex_energies(
            run_path, f"{complex_name}_solute"
        )
        e_solvent_raw, e_solvent_bsse, has_solvent_bsse = get_complex_energies(
            run_path, f"{complex_name}_solvent"
        )

        e_complex = _pick_energy(e_raw, e_bsse, has_bsse, use_bsse)
        e_solute = _pick_energy(
            e_solute_raw, e_solute_bsse, has_solute_bsse, use_bsse
        )
        e_solvent = _pick_energy(
            e_solvent_raw, e_solvent_bsse, has_solvent_bsse, use_bsse
        )

        formula = _pretty_formula(complex_name)
        if e_solute is not None:
            rows[(formula, "E(solute)")] = e_solute
        if e_solvent is not None:
            rows[(formula, "E(solvent)")] = e_solvent
        if e_complex is not None:
            rows[(formula, "E(complex)")] = e_complex
        if (
            e_complex is not None
            and e_solute is not None
            and e_solvent is not None
        ):
            rows[(formula, "I(solvent,solute)")] = (
                e_complex - e_solute - e_solvent
            )
...
def analyze_runs(base_dir: str = DEFAULT_BASE_DIR) -> pd.DataFrame:
    ...
    all_rows: Dict[Tuple[str, str, str, str, str], Dict[str, float]] = {}
    column_order: List[str] = []
    method_map: Dict[str, str] = {}
    source_map: Dict[str, str] = {}
    component_rank = {name: i for i, name in enumerate(DE_COMPONENTS)}
    ...
    records = sorted(
        records,
        key=lambda rec: (
            str(rec["from_formula"]),
            str(rec["to_formula"]),
            str(rec["balanced_reaction"]),
            component_rank.get(str(rec["component"]), 10**9),
        ),
    )
...
def analyze_mre(base_dir: str = DEFAULT_BASE_DIR) -> pd.DataFrame:
    """Solute/solvent two-body decomposition for every complex."""
    run_sources = _collect_run_sources(base_dir)
    if not run_sources:
        return pd.DataFrame()

    all_rows: Dict[Tuple[str, str], Dict[str, float]] = {}
    column_order: List[str] = []
    method_map: Dict[str, str] = {}
    source_map: Dict[str, str] = {}
    component_rank = {name: i for i, name in enumerate(MRE_COMPONENTS)}
    ...
    for run_name, source_base_dir, run_path in run_sources:
        ...
        for use_bsse in (False, True):
            row_values = _collect_mre_rows_for_run(
                run_path,
                source_base_dir=source_base_dir,
                use_bsse=use_bsse,
            )
            ...
            for row_key, value in row_values.items():
                all_rows.setdefault(row_key, {})
                all_rows[row_key][column_name] = value

    if not all_rows:
        return pd.DataFrame()

    records: List[Dict[str, object]] = []
    for (formula, component), values in all_rows.items():
        record: Dict[str, object] = {
            "formula": formula,
            "component": component,
        }
    ...
    records = sorted(
        records,
        key=lambda rec: (
            _complex_sort_key(str(rec["formula"])),
            str(rec["formula"]),
            component_rank.get(str(rec["component"]), 10**9),
        ),
    )
...
def _print_run_legend(df: pd.DataFrame) -> None:
...
if __name__ == "__main__":
    ...
    df_mre = analyze_mre(base_dir="SP_init")
    ...
    if df.empty:
        ...
        print(
            df[
                ["reaction_type", "balanced_reaction", "component"] + value_cols
            ].to_string(index=False)
        )

    if df_mre.empty:
        print(
            "\nNo MRE decomposition found "
            "(need *_full, *_solute_full, *_solvent_full)."
        )
    else:
        mre_file = "MRE.csv"
        df_mre.to_csv(mre_file, index=False)
        print(f"\nMRE results saved to {mre_file}")
        if df.empty:
            _print_run_legend(df_mre)
        print(
            "\nMRE: E(complex) = E(solute) + E(solvent) + I(solvent,solute) "
            "(kcal/mol):"
        )
        print(df_mre.to_string(index=False))
