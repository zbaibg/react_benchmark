...
``max_Zn-Omin_NO3(Å)`` — for each ``NO3`` residue, the shortest Zn–O distance; the reported value is the maximum of
those minima over all nitrate ions. ``max_Zn-O_MeOH(Å)`` — for each ``MOH`` residue, one Zn–O distance (``O1`` or
sole O); the maximum over methanol molecules. Both in Å (three decimals). ``Flag`` includes ``NO3_far`` / ``MeOH_far`` if
those metrics exceed ``ZN_O_NO3_SUSPECT`` / ``ZN_O_SUSPECT`` (2.5 Å by default). ``no_NO3_MeOH`` in ``Flag`` only when
the topology has no ``NO3`` and no ``MOH`` residues (not merely N/A distances). If ``min.out`` contains DL-FIND abort text
...
ZN_O_NO3_SUSPECT = 2.5  # Å → Flag includes NO3_far if max_Zn-Omin_NO3 exceeds this
ZN_O_SUSPECT = 2.5  # Å → Flag includes MeOH_far if max Zn–O(MeOH) exceeds this

TSV_COL_MAX_ZN_ONO3 = "max_Zn-Omin_NO3(Å)"
...
def analyze_zn_ligand_last_frame(
    path_xyz: Path,
) -> tuple[float | None, float | None, str, bool, bool]:
    """Parse last path.xyz frame via zif_meoh_assign_name.

    Returns ``(max_Zn-Omin_NO3, max_Zn-O_MeOH, error_tag, has_NO3_residue, has_MOH_residue)``. For each ``NO3``
    residue, the closest Zn–O is taken; the first value is the maximum of those per-ion minima. For each ``MOH``
    residue, Zn–O is taken (``O1`` or any O); the second value is the maximum over methanol molecules. The two
    booleans indicate whether any ``NO3`` / ``MOH`` residue exists in the assigned topology (for ``no_NO3_MeOH``).

    error tag is empty on success; otherwise a short reason (e.g. ``no_Zn``, ``Zn!=1``, ``no_zif_meoh_module``).
    On error, both booleans are False.
    """
    zma = _import_zif_meoh_assign_name()
    if zma is None:
        return None, None, "no_zif_meoh_module", False, False

    try:
        frame_txt = read_last_xyz_frame_text(path_xyz)
    except (OSError, ValueError) as e:
        return None, None, f"path_xyz:{e!s}"[:120], False, False
    ...
    try:
        ...
        return None, None, f"xyz_to_mda:{e!s}"[:120], False, False
    finally:
        try:
            os.unlink(tmp)
        except OSError:
    ...
    if len(zn_atoms) == 0:
        return None, None, "no_Zn", False, False
    if len(zn_atoms) != 1:
        return None, None, "Zn!=1", False, False

    zn_pos = zn_atoms.positions[0]
    no3_residues = [r for r in u.residues if r.resname == "NO3"]
    moh_residues = [r for r in u.residues if r.resname == "MOH"]
    has_no3 = len(no3_residues) > 0
    has_moh = len(moh_residues) > 0

    # Each nitrate: min Zn–O among its oxygens; then max over all NO3 ions.
    no3_per_ion_min_zno: list[float] = []
    for res in no3_residues:
        oxy = res.atoms.select_atoms("element O")
        if len(oxy) == 0:
            continue
        min_d: float | None = None
        for o in oxy:
            d = _dist3(zn_pos, o.position)
            if min_d is None or d < min_d:
                min_d = d
        if min_d is not None:
            no3_per_ion_min_zno.append(min_d)

    # Each MeOH: one Zn–O; report max over methanol molecules.
    ...
    max_no3 = max(no3_per_ion_min_zno) if no3_per_ion_min_zno else None
    max_moh = max(moh_zn_os) if moh_zn_os else None
    return max_no3, max_moh, "", has_no3, has_moh


def geometry_tsv_fields(sys_dir: Path, system_rel: str, na: str) -> dict[str, str]:
    """TSV columns from path.xyz last frame: max_Zn-Omin_NO3, max Zn–O(MeOH), Flag."""
    if "_monomer" in system_rel:
        return {
            TSV_COL_MAX_ZN_ONO3: na,
    ...
    try:
        max_no3, max_moh, err, has_no3, has_moh = analyze_zn_ligand_last_frame(path_xyz)
    except Exception as e:  # noqa: BLE001
        return {
            TSV_COL_MAX_ZN_ONO3: na,
            TSV_COL_MAX_ZN_OMOH: na,
    ...
    if not has_no3 and not has_moh:
        return {
            TSV_COL_MAX_ZN_ONO3: na,
            TSV_COL_MAX_ZN_OMOH: na,
            TSV_COL_FLAG: "no_NO3_MeOH",
        }

    flags: list[str] = []
    if max_no3 is not None and max_no3 > ZN_O_NO3_SUSPECT:
        flags.append("NO3_far")
    if max_moh is not None and max_moh > ZN_O_SUSPECT:
        flags.append("MeOH_far")

    return {
        TSV_COL_MAX_ZN_ONO3: na if max_no3 is None else f"{max_no3:.3f}",
