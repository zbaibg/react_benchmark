...
SKIP_PRMTOP_HARMONICRST_NDX_GENERATION=true

##### Settings regarding creating MOL topology
MOL_xyz=%MOL_XYZ%
RST_SOURCE_DIR=/home/zbai29/data/qmmm_test/react_benchmark/lig_exchange/MIMH_PBE_STRUC/minimize/run8/Zn_monomer
PRMTOP_SOURCE_DIR=/home/zbai29/data/qmmm_test/react_benchmark/lig_exchange/MIMH_PBE_STRUC/minimize/run8/Zn_monomer
charge_MOL=2
MOL_mask=":*"
...
&dftbplus
  ...
  mdftb= .true.,
  mdftb_scale= .true.,
  skroot= '/home/zbai29/soft/3ob-3-1_modify_O',
/
&xtb
  qm_level='DFTB3-D3',
  maxiter=250,
...
apptainer exec -e \\
  ...
  /home/zbai29/JR/soft/apptainer/amber_mdftb.sif bash -c "\\
    ...
    source /home/zbai29/soft/ambertools25_src_425dd98/install/amber.sh &&
    ulimit -s unlimited &&
...
function main() {
    ...
    if [ "${SKIP_PRMTOP_HARMONICRST_NDX_GENERATION}" != true ]; then
        ...
        if [ -d "$RST_SOURCE_DIR" ]; then
            ...
            if [ -f "$RST_SOURCE_DIR/box_orig.inpcrd" ]; then
                cp "$RST_SOURCE_DIR/box_orig.inpcrd" ./min_orig.rst
            else
                echo "Warning: box_orig.inpcrd not found in $RST_SOURCE_DIR. Trying to find any .rst"
                exit 1
            fi
        else
