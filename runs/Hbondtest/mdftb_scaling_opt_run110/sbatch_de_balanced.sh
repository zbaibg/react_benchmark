#!/bin/bash
#SBATCH --job-name=mdftb_de_bal
#SBATCH --output=mdftb_scaling_opt_run110/slurm-de-balanced-%j.out
#SBATCH --error=mdftb_scaling_opt_run110/slurm-de-balanced-%j.err
...
export MDFTB_OPT_RESULTS_DIR="$PWD/mdftb_scaling_opt_run110/results_de_balanced"
mkdir -p "$MPLCONFIGDIR" "$MDFTB_OPT_RESULTS_DIR"

if [ -d /scratch ]; then
    export MDFTB_OPT_TMP_DIR="/scratch/${SLURM_JOB_ID}/mdftb_de_balanced_tmp"
else
    export MDFTB_OPT_TMP_DIR="$PWD/mdftb_scaling_opt_run110/tmp/${SLURM_JOB_ID}_de_balanced"
fi
...
echo "MODE: de"
echo "OBJECTIVE: rmse_slope_mean"
echo "DE_MAXITER: ${DE_MAXITER}"
echo "DE_POPSIZE: ${DE_POPSIZE}"
echo "RMSE_SCALE: ${RMSE_SCALE}"
...
python mdftb_scaling_opt_run110/optimize_mdftb_scalings.py \
    --mode de \
    --objective rmse_slope_mean \
    --rmse-scale "$RMSE_SCALE" \
