#!/bin/bash
#SBATCH --job-name=mdftb_sm_opt
#SBATCH --output=mdftb_scaling_opt_run110/slurm-slope-mean-%j.out
#SBATCH --error=mdftb_scaling_opt_run110/slurm-slope-mean-%j.err
...
export MDFTB_OPT_RESULTS_DIR="$PWD/mdftb_scaling_opt_run110/results_slope_mean"
mkdir -p "$MPLCONFIGDIR" "$MDFTB_OPT_RESULTS_DIR"

if [ -d /scratch ]; then
    export MDFTB_OPT_TMP_DIR="/scratch/${SLURM_JOB_ID}/mdftb_slope_mean_opt_tmp"
else
    export MDFTB_OPT_TMP_DIR="$PWD/mdftb_scaling_opt_run110/tmp/${SLURM_JOB_ID}_slope_mean"
fi
