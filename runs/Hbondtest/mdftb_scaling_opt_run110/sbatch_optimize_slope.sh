#!/bin/bash
#SBATCH --job-name=mdftb_slope_opt
#SBATCH --output=mdftb_scaling_opt_run110/slurm-slope-%j.out
#SBATCH --error=mdftb_scaling_opt_run110/slurm-slope-%j.err
...
export MDFTB_OPT_RESULTS_DIR="$PWD/mdftb_scaling_opt_run110/results_slope"
mkdir -p "$MPLCONFIGDIR" "$MDFTB_OPT_RESULTS_DIR"

if [ -d /scratch ]; then
    export MDFTB_OPT_TMP_DIR="/scratch/${SLURM_JOB_ID}/mdftb_slope_opt_tmp"
else
    export MDFTB_OPT_TMP_DIR="$PWD/mdftb_scaling_opt_run110/tmp/${SLURM_JOB_ID}_slope"
fi
