#!/bin/bash
#SBATCH --job-name=mdftb_scale_opt
#SBATCH --output=mdftb_scaling_opt_run110/slurm-%j.out
#SBATCH --error=mdftb_scaling_opt_run110/slurm-%j.err
...
if [ -d /scratch ]; then
    export MDFTB_OPT_TMP_DIR="/scratch/${SLURM_JOB_ID}/mdftb_scaling_opt_tmp"
else
    export MDFTB_OPT_TMP_DIR="$PWD/mdftb_scaling_opt_run110/tmp/${SLURM_JOB_ID}"
fi
