...
# ============ Defaults (used when run_configs.yaml omits distribute_tasks or keys) ============
DEFAULT_MAX_QM_WORKERS = 40
DEFAULT_MAX_OTHER_WORKERS = 300
DEFAULT_QM_CPUS_PER_TASK = 16
DEFAULT_OTHER_CPUS_PER_TASK = 1
# ============================================================================================

_RUN_KEY = re.compile(r"^run\d+$")


def _int_from_config(raw, default):
    if raw is None:
        return default
    try:
        return int(raw)
...
def load_run_config(config_path):
    """Load QM run list and distribute_tasks settings from run_configs.yaml.

    Returns (qm_runs, settings) where settings keys: max_qm_workers, max_other_workers,
    qm_cpus_per_task, other_cpus_per_task.
    """
    settings = {
        "max_qm_workers": DEFAULT_MAX_QM_WORKERS,
        "max_other_workers": DEFAULT_MAX_OTHER_WORKERS,
        "qm_cpus_per_task": DEFAULT_QM_CPUS_PER_TASK,
        "other_cpus_per_task": DEFAULT_OTHER_CPUS_PER_TASK,
    }
    ...
    dt = data.get("distribute_tasks")
    if isinstance(dt, dict):
        settings["max_qm_workers"] = _int_from_config(
            dt.get("max_qm_workers"), DEFAULT_MAX_QM_WORKERS
        )
        settings["max_other_workers"] = _int_from_config(
            dt.get("max_other_workers"), DEFAULT_MAX_OTHER_WORKERS
        )
        settings["qm_cpus_per_task"] = _int_from_config(
            dt.get("qm_cpus_per_task"), DEFAULT_QM_CPUS_PER_TASK
        )
        settings["other_cpus_per_task"] = _int_from_config(
            dt.get("other_cpus_per_task"), DEFAULT_OTHER_CPUS_PER_TASK
        )

    qm_runs = []
    for key, val in data.items():
        if not isinstance(key, str) or not _RUN_KEY.match(key):
            continue
        if not isinstance(val, dict):
            continue
        tmpl = val.get("orca_template")
        if tmpl is None:
            continue
        ...
        if not tmpl:
            continue
        qm_runs.append(key)
    qm_runs.sort(key=lambda name: int(name[3:]))

    return qm_runs, settings


def load_qm_runs_from_run_configs(config_path):
    """QM runs that have non-empty ``orca_template`` (same file read as ``load_run_config``)."""
    qm_runs, _settings = load_run_config(config_path)
    return qm_runs
...
if [ "$QM_COUNT" -gt 0 ]; then
    LIMIT=$((QM_COUNT - 1))
    echo "Submitting $QM_COUNT workers for QM methods ({qm_cpus_per_task} cores, array 0-$LIMIT)..."
...
if [ "$OTHER_COUNT" -gt 0 ]; then
    LIMIT=$((OTHER_COUNT - 1))
    echo "Submitting $OTHER_COUNT workers for other runs ({other_cpus_per_task} cores, array 0-$LIMIT)..."
...
def generate_worker_qm_slurm(target_dir, script_name, qm_cpus_per_task):
...
def generate_worker_other_slurm(target_dir, script_name, other_cpus_per_task):
...
def process_single_directory(target_subdir, script_dir, script_name, qm_runs, dist_settings):
    ...
    try:
        ...
        # Write task files
        max_qm = dist_settings["max_qm_workers"]
        max_other = dist_settings["max_other_workers"]
        n_qm = write_task_files(target_dir, qm_tasks, "qm", max_qm)
        print(f"Generated {n_qm} task lists for QM methods (max {max_qm}).")
        
        n_other = write_task_files(target_dir, other_tasks, "other", max_other)
        print(f"Generated {n_other} task lists for other runs (max {max_other}).")
        ...
        generate_submit_script(
            ...
            max_qm,
            max_other,
            dist_settings["qm_cpus_per_task"],
            dist_settings["other_cpus_per_task"],
        )
        generate_worker_qm_slurm(target_dir, script_name, dist_settings["qm_cpus_per_task"])
        generate_worker_other_slurm(
            target_dir, script_name, dist_settings["other_cpus_per_task"]
        )
...
def main():
    ...
    run_config_path = os.path.join(script_dir, "run_configs.yaml")
    qm_runs, dist_settings = load_run_config(run_config_path)
    print(f"QM runs (orca_template in {run_config_path}): {', '.join(qm_runs) if qm_runs else '(none)'}")
    print(
        "distribute_tasks: "
        f"max_qm={dist_settings['max_qm_workers']}, max_other={dist_settings['max_other_workers']}, "
        f"qm_cpus={dist_settings['qm_cpus_per_task']}, other_cpus={dist_settings['other_cpus_per_task']}"
    )

    print(f"\\n{'#'*60}")
    print(
        f"# Processing {len(target_subdirs)} "
        f"{'directory' if len(target_subdirs) == 1 else 'directories'}"
    )
    ...
    for target_subdir in target_subdirs:
        if process_single_directory(
            target_subdir, script_dir, script_name, qm_runs, dist_settings
        ):
            success_count += 1
        else:
